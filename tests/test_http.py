...
def test_private_http_requires_explicit_opt_in():
    try:
        validate_destination_url("http://192.168.50.25:3200/api/health")
...
def test_private_http_can_be_enabled_for_internal_source():
    validate_destination_url("http://192.168.50.25:3200/api/health", allow_private=True)


def test_private_http_does_not_bypass_outbound_ssrf_policy(monkeypatch):
    monkeypatch.setattr("paperclip_notifier.http.socket.getaddrinfo", lambda *args, **kwargs: [(None, None, None, None, ("192.168.50.25", 0))])
    try:
...
def test_paperclip_client_opts_into_private_source(monkeypatch):
    ...
    PaperclipClient("http://192.168.50.25:3200", "secret", "company").health()
...
def test_paperclip_activity_uses_larger_bounded_response_limit(monkeypatch):
    ...
    PaperclipClient("http://192.168.50.25:3200", "secret", "company").activity()
...
def test_paperclip_attention_uses_active_feed_without_unscoped_all(monkeypatch):
    ...
    PaperclipClient("http://192.168.50.25:3200", "secret", "company").attention_all()
