...
6. Confirm the Paperclip API address. The template default is `http://192.168.50.25:3200`; use `http://<paperclip-container>:3100` only when both containers share a user-defined Docker network.
...
11. Create one controlled Paperclip test activity and confirm only the intended destination receives it with a link beginning `https://paperclip.example.com/`.
